...
import org.immregistries.mqe.hl7util.parser.profile.ack.generated.AckConformanceProfile.Segments;
 ...
 *
 */
public class MessageProfileSource implements MessageProfile {

  private static final Logger LOGGER = LoggerFactory.getLogger(MessageProfileSource.class);
  ...
  public MessageProfileSource(String type) {
    switch (type) {
      case "ACK":
        this.populateAckProfileMap("/ACK-Z23_Profile.xml");
        break;
      case "QBP":
        this.populateAckProfileMap("/QBP-Z44_Profile.xml");
        break;
      default:
...
//		InputStream is = MessageProfileXML.class.getResourceAsStream("/immunization-profile.xml");
    InputStream is = MessageProfileSource.class.getResourceAsStream(fileName);
    ...
    if (is == null) {
      ...
      try {
        JAXBContext jaxbContext = JAXBContext
            .newInstance(AckConformanceProfile.class, AckObjectFactory.class);
        ...
        for (AckConformanceProfile.Datatypes.Datatype dt : hcp.getDatatypes().getDatatype()) {
          if (this.dataTypeComplexityMap.get(dt.getName()) == null) {
            ...
            dataTypeComplexityMap
                .put(dt.getName(), (complex ? FieldComplexity.COMPLEX : FieldComplexity.SIMPLE));
          }
...
//		FIELD_COMPLEXITY_MAP.put(s.getName() + "-0", FieldComplexity.SIMPLE);
   ...
   * @param datatype
   */
  private void mapDataLocation(boolean complex, String fieldName, String fieldDesc,
      String datatype) {
    ...
    if (cplx == null) {
      dataTypeComplexityMap
          .put(datatype, (complex ? FieldComplexity.COMPLEX : FieldComplexity.SIMPLE));
